...
def serviciosList(request):
    prosi = models.RegistroProS.objects.filter(
        ciudadano__id=request.user.pk).values('fecha', 'tramitado')
    prono = models.RegistroProN.objects.filter(
        ciudadano__id=request.user.pk).values('fecha', 'tramitado')
    tarjeta = models.TarjetaPro.objects.filter(
        ciudadano__id=request.user.pk).values('fecha', 'tramitado')
    pasaporte = models.Pasaporte.objects.filter(
        ciudadano__id=request.user.pk).values('fecha', 'tramitado')
